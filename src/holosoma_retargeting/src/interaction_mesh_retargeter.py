...
class InteractionMeshRetargeter:
    ...
    def __init__(
        ...
        # Debug: verify model/config alignment to avoid false dynamic-object detection
        print("qpos_len:", self.robot_data.qpos.shape[0])
        print("ROBOT_DOF:", self.task_constants.ROBOT_DOF)
        ...
        for i, name in enumerate(joint_names):
            if not name:
                continue
            # Skip free joints (e.g., named floating base in 23dof) to keep qpos alignment.
            if self.robot_model.jnt_type[i] == mujoco.mjtJoint.mjJNT_FREE:
                continue
            actuated_joints.append((i, name))
        ...
        # Apply manual bounds overrides. Indices are interpreted as full-qpos indices
        # (so they keep working even when q_a_init_idx excludes the floating base).
        for idx_str, value in self.task_constants.MANUAL_LB.items():
            idx = int(idx_str)
            hits = np.where(self.q_a_indices == idx)[0]
            if hits.size:
                self.q_a_lb[hits[0]] = float(value)
        for idx_str, value in self.task_constants.MANUAL_UB.items():
            idx = int(idx_str)
            hits = np.where(self.q_a_indices == idx)[0]
            if hits.size:
                self.q_a_ub[hits[0]] = float(value)

        # Prevent too much waist twist
        self.Q_diag = np.zeros(self.nq_a) * 1e-3
        for idx_str, value in self.task_constants.MANUAL_COST.items():
            idx = int(idx_str)
            hits = np.where(self.q_a_indices == idx)[0]
            if hits.size:
                self.Q_diag[hits[0]] = float(value)
    ...
    def retarget_motion(
        ...
        if q_nominal_list is not None:
            ...
            q_a_init_arr = np.asarray(q_a_init, dtype=float)
            if q_a_init_arr.shape == (self.nq,):
                # Full qpos provided (base + joints [+ optional extras]); copy as-is.
                q_locked_list[0, :] = q_a_init_arr
            elif q_a_init_arr.shape == (len(self.q_a_indices),):
                # Only the optimized slice provided; place into qpos.
                q_locked_list[0, self.q_a_indices] = q_a_init_arr
            else:
                raise ValueError(
                    f"q_a_init has shape {q_a_init_arr.shape}, expected ({len(self.q_a_indices)},) or ({self.nq},)"
                )
    ...
    def solve_single_iteration(
        ...
        # Linear equality
        # Note: J_L is already reduced to the optimization slice (nq_a columns),
        # so we must not index it again with full-qpos indices (self.q_a_indices).
        ...
        if (self.q_a_init_idx < 12) and self.activate_foot_sticking:
            ...
            for key, J_WF in J_WF_dict.items():
                ...
                if apply_left or apply_right:
                    ...
                    # J_WF is already reduced to nq_a columns in _calc_manipulator_jacobians().
                    Jxy = J_WF[:2, :]  # (2 x nq_a)
                    constraints += [
                        Jxy @ dqa >= p_lb[:2],
                        Jxy @ dqa <= p_ub[:2],
